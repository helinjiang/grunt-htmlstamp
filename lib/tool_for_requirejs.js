 ...
 */
exports.getInfo = function (type, pathObj, fileArr, dataMainUrl, configUrl, baseUrl) {
    var fMap = {},
        map,
        mapExternal,
        dataMainObj,
 ...
 * @param {RegExp} regPathsStr  用于匹配paths的正则表达式
 * @param {Object} mapPathsObj paths对应的对象
 * @param {String} savePath 新文件保存的路径
 */
exports.saveRequireJSConfig = function (grunt, type, content, regPathsStr, mapPathsObj, savePath) {
    var newContent = content,
        urlArgsReg = /urlArgs\s*:.*,/g;

    if (util.isEmbed(type)) {
        // 将新的paths替换原config.js文件中的paths，同时移除urlArgs，因为这个参数是在开发场景时才使用
        newContent = newContent.replace(regPathsStr, "paths:" + JSON.stringify(mapPathsObj)).replace(urlArgsReg, '');
    } else {
        // 如果没有urlArgs字段，则追加之
...
exports.saveRequireJSDataMain = function (grunt, type, dataMainObj, configJsObj) {
    ...
    if (util.isEmbed(type)) {
        ...
        if (dataMainContent.match(reg2)) {
            ...
            // 如果reg2没匹配，则说明是无.js结尾的相对路径写法，此时就只能替换其相对于html的地址了
